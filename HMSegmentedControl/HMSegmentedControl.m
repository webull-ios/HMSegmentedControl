...
- (void)commonInit {
    ...
    self.spotMap = [NSMutableArray new];
    self.spotLayers = [NSMutableArray new];
}
...
- (void)setShowSpot:(NSUInteger)index isShow:(BOOL)isShow {
    BOOL originShow = NO;
    for (NSString *num in self.spotMap) {
        if ([[NSString stringWithFormat:@"%lu", (unsigned long)index] isEqualToString:num]) {
            originShow = YES;
            [self.spotMap removeObject:num];
            break;
        }
    }
    if (originShow != isShow) {
        if (isShow) {
            [self.spotMap addObject:[NSString stringWithFormat:@"%lu", (unsigned long)index]];
        }
        for (NSInteger i = 0; i < self.spotLayers.count; i++) {
            CALayer *spotLayer = self.spotLayers[i];
            if (i == index) {
                if (isShow) {
                    spotLayer.backgroundColor = [UIColor redColor].CGColor;
                } else {
                    spotLayer.backgroundColor = [UIColor clearColor].CGColor;
                }
            }
        }
    }
...
- (void)drawRect:(CGRect)rect {
    ...
    [self.spotLayers removeAllObjects];
...
//            CGFloat y = roundf((CGRectGetHeight(self.frame) - selectionStyleNotBox * self.selectionIndicatorHeight) / 2 - stringHeight / 2 + self.selectionIndicatorHeight * locationUp);
            ...
            spotLayer.frame = CGRectMake(rect.origin.x + rect.size.width/2 + [self measureTitleAtIndex:idx].width/2 + 2,
                                         rect.origin.y,
                                         5,
                                         5);
            [self.scrollView.layer addSublayer:spotLayer];
            BOOL needShowSpot = NO;
            for (NSString *num in self.spotMap) {
                if ([[NSString stringWithFormat:@"%lu", (unsigned long)idx] isEqualToString:num]) {
                    needShowSpot = YES;
                    break;
                }
            }
            ...
            [self.spotLayers addObject:spotLayer];
